...
export function ParentRegisterPage() {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      ...
      // Continue Flow A: the newly-verified guardian adds their first student.
      navigate('/register/add-student', { replace: true });
    } catch (err) {
    ...
    >
      <form onSubmit={onSubmit} className="flex flex-col gap-unit-md" noValidate>
        ...
        <Button type="submit" size="large" disabled={submitting} className="mt-unit-sm w-full">
          {submitting ? t('auth.actions.submitting') : t('auth.register.submit')}
        </Button>
      </form>
    </AuthLayout>
  );
